    ...
    <div class="grid grid-cols-1 sm:grid-col-2 md:grid-cols-3 lg:grid-cols-4 gap-4 m-6">
        ...
        @foreach ($productos as $producto)
            <div class="card bg-base-100 w-85 shadow-xl">
                ...
                <figure>
                    <img src="https://loremflickr.com/250/250/motorcycle,{{ urlencode($producto->nombre) }}?lock={{ $producto->id }}"
                        alt="{{ $producto->nombre }} " />
                </figure>
                <div class="card-body">
                    <h2 class="card-title">
                        <div class="label-name">{{ $producto->Nombre }}</div>
                    </h2>
                    <h2 class="card-title">
                        <div class="badge label-price">$ {{ $producto->Precio }}</div>
                    </h2>
